...
@Component({
  selector: 'app-step-addition-progress',
  templateUrl: './step-addition-progress.component.html',
  styleUrls: ['./step-addition-progress.component.css']
})

export class StepAdditionProgressComponent implements OnInit {
  ...
  getTestStepInfo(): void {
    ...
    this.isLoaded = true;
    this.testMappingService.getAllTestSteps(this.selectedOrg, this.clientProjectId)
      .subscribe(
        testCaseDefinitions => {
  ...
  noData() {
    d3.select('#TopLineChartDiv').selectAll('*').remove();
    if (this.data.length > 0) {
      d3.select('#TopLineChartDiv').append('mat-card').text('No Test Steps were uploaded since the selected Start Date: ' + this.inputStartDate.toDateString() + ' for the selected project. Last Uploaded: ' + new Date(d3.max(this.data, function (d) { return d.createdDate; })).toDateString() + '.');
    } else {
      d3.select('#TopLineChartDiv').append('mat-card').text('No Test Steps were uploaded since the selected Start Date: ' + this.inputStartDate.toDateString() + ' for the selected project.');
    }
  }
  ...
  showUserDateBar() {
    ...
    function tabulate(data, columns) {
      let table = d3.select('#userNameBar').append('table').style('float', 'left').style('margin', '10px').style('border-collapse', 'collapse')
        .style('font-family', 'arial, sans-serif').style('width', '80%');
